...
# Delete table
#cur.execute('DROP TABLE snake_score;')

conn.commit()

'''# Create a new table
cur.execute("""CREATE TABLE snake_score (
            nick VARCHAR(255),
            fscore VARCHAR(255)
);
""")'''
...
cur.execute(f"SELECT nick FROM snake_score WHERE nick = '{nick}';")
existing_nick = cur.fetchone()

if existing_nick:
    cur.execute(f"SELECT fscore FROM snake_score WHERE nick = '{nick}';")
    hisfscore = cur.fetchone()
    ress = hisfscore[0]
    print(f"Никнейм '{nick}' уже существует в базе данных и его результат: {ress}.")
    done = True

if not done:
    print(f"Добро пожаловать, {nick}! Игра начинается...")
...
while not done:
    ...
    if snake.body[0].x >= WIDTH / CELL or snake.body[0].x < 0 or snake.body[0].y >= HEIGHT / CELL or snake.body[0].y < 0:
        ...
        # Create new students
        cur.execute(f"""INSERT INTO snake_score (nick, fscore) VALUES 
                 ('{nick}', '{fscore}');
        """)
        conn.commit()

        cur.execute('SELECT nick, fscore FROM snake_score')
        done = True
