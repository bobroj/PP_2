...
INC_SPEED = pygame.USEREVENT + 5  #создание ивента для увеличения скорости
...
# bronze coin
class BCoin(pygame.sprite.Sprite):   
    def __init__(self):
        super().__init__()
...
while not done:
    ...
    # taking coins:
    if pygame.sprite.spritecollideany(P1, gcoins):
        cscore += 3
        C1.rect.center = (random.randint(50, WIDTH - 50), 35)
        isup = True

    if cscore%10 == 0 and isup == True:
        SPEED += 1
        isup = False
    ...
    if pygame.sprite.spritecollideany(P1, scoins):
        cscore += 2
        C2.rect.center = (random.randint(50, WIDTH - 50), 35)
        isup = True
    ...
    if pygame.sprite.spritecollideany(P1, bcoins):
        cscore += 1
        C3.rect.center = (random.randint(50, WIDTH - 50), 35)
        isup = True
